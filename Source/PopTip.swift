...
/* Enum to set poptip alignment with respect to container view */
public enum PopTipViewAlignment {
    /// Align poptip view to left of container view
    case left
    /// Align poptip view to right of container view
    //case right
    /// Align poptip view to center of container view
    case center
}
...
open class PopTip: UIView {
    ...
    /// poptip view alignment
    open var viewAlignment = PopTipViewAlignment.center
    /// left padding for poptip view
    open var leftPadding = 0.0
    /// right padding for poptip view
    open var rightPadding = 0.0
    ...
    internal func setupVertically() -> (CGRect, CGPoint) {
        ...
        case .left:
            ...
            // arrow postion to right of leftPadding
            arrowPosition.x = frame.origin.x + leftPadding
        case .center:
            if frame.size.width + frame.origin.x >= containerView.frame.width {
                //add padding at right so that bubble doesn't touches the view
                frame.origin.x -= rightPadding
                arrowPosition.x += 16
            } else if frame.origin.x == 0 {
                //add padding at left so that bubble doesn't touches the view
                frame.origin.x += leftPadding
                arrowPosition.x -= 16
            }
        }
